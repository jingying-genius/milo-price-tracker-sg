        ...
        with sync_playwright() as p:
            try:
                ...
                try:
                    ...
                    if not containers:
                        print(f"  ⚠️  No containers found, trying text search...")
                        all_divs = soup.find_all('div')
                        containers = [div for div in all_divs if 'milo' in div.get_text().lower()][:max_products * 2]
                    ...
                    for container in containers[:max_products]:
                        try:
                            ...
                            if not name or 'milo' not in name.lower():
                                continue
...
class ShopeeScraper(BaseScraper):
    ...
    def scrape(self, headless=True, max_products=10):
        ...
        with sync_playwright() as p:
            try:
                ...
                try:
                    ...
                    time.sleep(8)  # Longer wait for JavaScript
                    ...
                    if not containers:
                        print(f"  ⚠️  No product containers found. Trying alternative approach...")
                        # Try finding by text
                        all_links = soup.find_all('a', href=True)
                        containers = [link.parent for link in all_links if 'milo' in link.get_text().lower()][:max_products]
                        print(f"  📦 Found {len(containers)} products by text search")
                    ...
                    for container in containers[:max_products]:
                        try:
                            ...
                            print(f"  ⚠️  Skipped product: {str(e)}")
                            continue
        ...
        flash_count = sum(1 for p in products if p.get('flash_sale', False))
